"use client";
import { useAddSubscription } from "../../../queries/programqueries"
...
export default function SubscriptionPlans() {
    ...
    const handleMutatioon = (data) => {
        const response = addSubscription.mutate(data);
        return;
    }
            ...
            <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                {pricingPlans.map((plan, i) => (
                    <div key={i} className={`relative border ${plan.popular ? 'border-[#fff200]' : 'border-gray-900'} bg-[#0a0a0a] p-10 rounded-3xl flex flex-col`}>
                        ...
                        <button onClick={() => handleMutatioon(plan.plan)} className={`${plan.popular ? 'bg-[#fff200] text-black' : 'bg-white text-black'} py-4 rounded-2xl font-black uppercase tracking-widest text-xs hover:scale-105 transition-transform`}>
                            Get Started Now
                        </button>
                    </div>
                ))}
